<section style="{{ $styles }}" class="{{ $classes }} s-team3 @if(isset($block['className'])) {{ $block['className'] }} @endif">
  <div class="container">
    <x-title :tag="$title['tag']" :position="$title['position']">{!! $title['value'] !!}</x-title>
    {!! $description !!}
    <div class="members">
      <x-slider name="team-3" :per="count($members) < 4 ? count($members) : 4" perMd="3" perSm="2" perXs="1" :modules="['navigation','pagination']"
