<section style="{{ $styles }};" class="{{ $classes }} s-hero5 {{ $block['className'] ?? '' }}">
  @if($image && count($image) === 1 && $image = $image[0])
    <div class="s-hero5__bg">
      ...
      <video src="{{ $image['video']['url'] }}" {{ $image['autoplay'] ? 'autoplay' : '' }} {{ $image['mute'] ? 'muted' : '' }} {{ $image['autoplay'] && !$image['mute'] ? 'muted video-unmute' : '' }} {{ $image['loop'] ? 'loop' : '' }} playsinline loading="lazy"></video>
